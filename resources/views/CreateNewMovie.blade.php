    ...
    <body class="bg-base-300">
        ...
        <div  class="flex flex-row justify-center gap-24 py-4 h-fit">
            ...
            <div class="flex flex-col items-center gap-16 bg-base-100 p-16 rounded-xl h-full ">
                ...
                <form action="{{$movieData? route('movies.edit_movie', $movieData->id) :route('movies.create_movie')}}" method="post" class=" flex-col flex gap-4">
                    @csrf
                    ...
                    <input id="imageUrlInput" type="text" name="url" placeholder="URL" value="{{$movieData? $movieData->url: ''}}" class="focus:border-[#ff6b00]  input input-bordered  w-full max-w-xs" />
